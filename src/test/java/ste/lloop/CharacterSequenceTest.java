...
public class CharacterSequenceTest {
    ...
    public void loop_with_step() {
        ...
        final StringBuilder sb = new StringBuilder();
        new CharacterSequence(string).from(0).to(10).step(2).loop((index, element) -> sb.append(index).append(element));
        then(sb.toString()).isEqualTo("0a2c4e6g8i10k");

        // from(10).to(0).step(2) -> 10,8,..,0
        sb.delete(0, sb.length());
        new CharacterSequence(string).from(10).to(0).step(2).loop((index, element) -> sb.append(index).append(element));
        then(sb.toString()).isEqualTo("10k8i6g4e2c0a");

        // step is zero, no loop
        sb.delete(0, sb.length());
        new CharacterSequence(string).from(0).to(10).step(0).loop((index, element) -> sb.append(index).append(element));
        then(sb.toString()).isEmpty();

        // negative step with no from/to
        sb.delete(0, sb.length());
        new CharacterSequence(string).step(-2).loop((index, element) -> sb.append(index).append(element));
        then(sb.toString()).isEqualTo("10k8i6g4e2c0a");
    }
